...
export default class AnyVirtualize extends PureComponent {
  ...
  componentWillUnmount() {
    if (this.scrollRef) {
      ...
      scrollbar.destroy(this.scrollRef);
  ...
  initialScrollbar = () => {
    if (this.scrollRef) {
      this.scrollbar = null;
      scrollbar.destroy(this.scrollRef);
      this.scrollbar = scrollbar.initialize(this.scrollRef, scrollbarOptions);
      this.scrollRef.addEventListener('ps-scroll-x', this.updateScrollIndex, false);
      this.resizeEvent();
    }
  }
